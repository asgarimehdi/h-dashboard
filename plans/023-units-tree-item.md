# Plan 023: Add Livewire coverage for units.tree-item partial via units.index
...
- **Effort**: M
- **Risk**: LOW
- **Depends on**: plans/plans/002-units-index.md.md
- **Category**: tests
- **Planned at**: commit `af29080`, 2026-09-03
- **Issue**: https://github.com/asgarimehdi/h-dashboard/issues/584

## Why this matters

The `units.tree-item (partial)` Livewire component currently has **no dedicated Livewire test
...
- **Component**: `units.tree-item (partial)` — single-file anonymous Livewire 4 class under
  `resources/views/livewire/` (see AGENTS.md: no `app/Livewire/*.php` files).
- **Route**: `/units (via units.index recursive include)` — protected by `organization` (see `routes/web.php`).
...
| Run focused | `XDEBUG_MODE=off php artisan test tests/Feature/UnitsTreeItemLivewireTest.php` | All pass |
...
- `tests/Feature/UnitsTreeItemLivewireTest.php` (CREATE)
...
```bash
ls resources/views/livewire/units/tree-item.blade.php
grep -n "tree-item" routes/web.php
...
Create `tests/Feature/UnitsTreeItemLivewireTest.php` following the exemplar pattern below
...
**Verify**: `XDEBUG_MODE=off php artisan test tests/Feature/UnitsTreeItemLivewireTest.php`
...
- S1: Auth+perm GET /units 200; root names render; leaf dot vs parent +/- toggle
- S2: toggle(id) expands -> children div; toggle again collapses; stop propagation keeps selectUnit untriggered
- S3: selectUnit(id) sets selected/current; detail pane updates
- S4: search substring -> matching node border-primary.
...
- E1: 3-level nesting indentation level*28px; isLast shortens branch line; unit without unitType no type span; empty children no toggle icon
- E2: Explicit-ID seeding + setval resync.
...
New tests to write in `tests/Feature/UnitsTreeItemLivewireTest.php` (model structure on
...
- [ ] Focused run `XDEBUG_MODE=off php artisan test tests/Feature/UnitsTreeItemLivewireTest.php` exits 0
