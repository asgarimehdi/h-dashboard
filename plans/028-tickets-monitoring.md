# Plan 028: Add Livewire coverage for tickets.monitoring
...
- **Issue**: https://github.com/asgarimehdi/h-dashboard/issues/589

## Why this matters

The `tickets.monitoring` Livewire component currently has **no dedicated Livewire test
...
- **Component**: `tickets.monitoring` — single-file anonymous Livewire 4 class under
  `resources/views/livewire/` (see AGENTS.md: no `app/Livewire/*.php` files).
- **Route**: `/monitoring` — protected by `view_all_tickets (+auth+unit_context)` (see `routes/web.php`).
...
| Run focused | `XDEBUG_MODE=off php artisan test tests/Feature/TicketsMonitoringLivewireTest.php` | All pass |
...
- `tests/Feature/TicketsMonitoringLivewireTest.php` (CREATE)
...
```bash
ls resources/views/livewire/tickets/monitoring.blade.php
grep -n "tickets.monitoring" routes/web.php
grep -rn "Livewire::test(\"tickets.monitoring\")" tests/ | head -5
```
...
Create `tests/Feature/TicketsMonitoringLivewireTest.php` following the exemplar pattern below
...
**Verify**: `XDEBUG_MODE=off php artisan test tests/Feature/TicketsMonitoringLivewireTest.php`
...
- S1: Guest 302, no-perm 403, authed 200 + header; empty -> empty state
- S2: statusFilter all/pending(created+forwarded)/accepted/completed
- S3: search subject OR ticket_code LIKE
- S4: Jalali dateFrom/dateTo -> Miladi startOfDay/endOfDay bounds on created_at
- S5: unitSearch >1 char loads filterUnits (can_receive_tickets, limit 10); <=1 empty
- S6: selectUnitForFilter sets selectedUnitId + clears search + resets page + currentUnit
- S7: showTicket in-scope loads relations + modal; out-of-scope error + closed
- S8: closeDetail resets
- S9: updated() resets page on filter change.

Use `Livewire::test('tickets.monitoring')` with `actingAs($user)`; assert state with
...
- E1: Invalid Jalali string (document behavior)
- E2: selectedUnitId deleted -> currentUnit null + fallback badge
- E3: 20/page latest() ordering.
...
New tests to write in `tests/Feature/TicketsMonitoringLivewireTest.php` (model structure on
...
- [ ] Focused run `XDEBUG_MODE=off php artisan test tests/Feature/TicketsMonitoringLivewireTest.php` exits 0
