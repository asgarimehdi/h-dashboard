# Plan 020: Add Livewire coverage for reports.map-no-boundary
...
- **Issue**: https://github.com/asgarimehdi/h-dashboard/issues/580

## Why this matters

The `reports.map-no-boundary` Livewire component currently has **no dedicated Livewire test
...
- **Component**: `reports.map-no-boundary` — single-file anonymous Livewire 4 class under
  `resources/views/livewire/` (see AGENTS.md: no `app/Livewire/*.php` files).
- **Route**: `/reports/map-no-boundary` — protected by `auth+unit_context` (see `routes/web.php`).
...
| Run focused | `XDEBUG_MODE=off php artisan test tests/Feature/ReportsMapNoBoundaryLivewireTest.php` | All pass |
...
- `tests/Feature/ReportsMapNoBoundaryLivewireTest.php` (CREATE)
...
```bash
ls resources/views/livewire/reports/map-no-boundary.blade.php
grep -n "map-no-boundary" routes/web.php
grep -rn "Livewire::test(\"reports.map-no-boundary\")" tests/ | head -5
```
...
Create `tests/Feature/ReportsMapNoBoundaryLivewireTest.php` following the exemplar pattern below
...
**Verify**: `XDEBUG_MODE=off php artisan test tests/Feature/ReportsMapNoBoundaryLivewireTest.php`
...
- S1: Guest 302, no unit_context -> select-context, authed 200 with header banner + 3 stat cards + <livewire:maps.map/>
- S2: showHelpModal toggles
- S3: allUnits/chartPayload filter has_coords
- S4: Stats counts match seeded (total/with/without coords)
- S5: Table shows only has_coords=false rows
- S6: Embedded maps.map present.

Use `Livewire::test('reports.map-no-boundary')` with `actingAs($user)`; assert state with
...
- E1: Empty accessibleUnitIds -> empty collection, zero stats, no error
- E2: All units with boundary -> empty table hidden
- E3: boundary_id null vs lat/lng present split correct
- E4: Out-of-scope units excluded
- E5: Cache key report:no_boundary:<md5> 300s TTL
- E6: No N+1 (eager unitType, region).
...
New tests to write in `tests/Feature/ReportsMapNoBoundaryLivewireTest.php` (model structure on
`tests/Feature/ActivityLogPageLivewireTest.php`):

- `test_guest_302`
- `test_no_context_redirect`
- `test_renders_stats`
- `test_help_modal`
- `test_counts_match`
- `test_table_visibility`
- `test_child_map_present`
...
- [ ] Focused run `XDEBUG_MODE=off php artisan test tests/Feature/ReportsMapNoBoundaryLivewireTest.php` exits 0
