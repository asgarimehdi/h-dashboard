# Plan 015: Add Livewire coverage for maps.unit
...
- **Issue**: https://github.com/asgarimehdi/h-dashboard/issues/575

## Why this matters

The `maps.unit` Livewire component currently has **no dedicated Livewire test
...
- **Component**: `maps.unit` — single-file anonymous Livewire 4 class under
  `resources/views/livewire/` (see AGENTS.md: no `app/Livewire/*.php` files).
- **Route**: `/maps/unit` — protected by `map (+auth+unit_context)` (see `routes/web.php`).
...
| Run focused | `XDEBUG_MODE=off php artisan test tests/Feature/MapsUnitLivewireTest.php` | All pass |
...
- `tests/Feature/MapsUnitLivewireTest.php` (CREATE)
...
```bash
ls resources/views/livewire/maps/unit.blade.php
grep -n "maps/unit" routes/web.php
grep -rn "Livewire::test(\"maps/unit\")" tests/ | head -5
```
...
Create `tests/Feature/MapsUnitLivewireTest.php` following the exemplar pattern below
...
**Verify**: `XDEBUG_MODE=off php artisan test tests/Feature/MapsUnitLivewireTest.php`
...
- S1: Guest 302, no-perm 403, authed 200 with unit boundary display + map
- S2: Unit selection updates boundary
- S3: Boundary visualization renders polygon.

Use `Livewire::test('maps/unit')` with `actingAs($user)`; assert state with
...
- E1: Unit without boundary -> empty state
- E2: Out-of-scope unit hidden
- E3: Cache behavior.
...
New tests to write in `tests/Feature/MapsUnitLivewireTest.php` (model structure on
...
- [ ] Focused run `XDEBUG_MODE=off php artisan test tests/Feature/MapsUnitLivewireTest.php` exits 0
