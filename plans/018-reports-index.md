# Plan 018: Add Livewire coverage for reports index/advanced page
...
- **Issue**: https://github.com/asgarimehdi/h-dashboard/issues/578

## Why this matters

The `reports (index)` Livewire component currently has **no dedicated Livewire test
...
- **Component**: `reports (index)` — single-file anonymous Livewire 4 class under
  `resources/views/livewire/` (see AGENTS.md: no `app/Livewire/*.php` files).
- **Route**: `/reports/tickets (+ siblings)` — protected by `auth+unit_context` (see `routes/web.php`).
...
| Run focused | `XDEBUG_MODE=off php artisan test tests/Feature/ReportsIndexLivewireTest.php` | All pass |
...
- `tests/Feature/ReportsIndexLivewireTest.php` (CREATE)
...
```bash
ls resources/views/livewire/reports/advanced.blade.php
grep -n "reports/" routes/web.php
grep -rn "Livewire::test(\"reports.advanced\")" tests/ | head -5
```
...
Create `tests/Feature/ReportsIndexLivewireTest.php` following the exemplar pattern below
...
**Verify**: `XDEBUG_MODE=off php artisan test tests/Feature/ReportsIndexLivewireTest.php`
...
- S1: Auth+unit_context 200 with report links
- S2: Guest 302
- S3: Missing unit_context -> select-context redirect
- S4: Report cards render with correct routes.

Use `Livewire::test('reports.advanced')` with `actingAs($user)`; assert state with
...
- E1: No reports available state
- E2: Scope filtering by accessible units.
...
New tests to write in `tests/Feature/ReportsIndexLivewireTest.php` (model structure on
...
- [ ] Focused run `XDEBUG_MODE=off php artisan test tests/Feature/ReportsIndexLivewireTest.php` exits 0
