# Plan 016: Add Livewire coverage for it.multi-gauge embedded component
...
- **Issue**: https://github.com/asgarimehdi/h-dashboard/issues/576

## Why this matters

The `it.multi-gauge (embedded)` Livewire component currently has **no dedicated Livewire test
...
- **Component**: `it.multi-gauge (embedded)` — single-file anonymous Livewire 4 class under
...
| Run focused | `XDEBUG_MODE=off php artisan test tests/Feature/ItMultiGaugeLivewireTest.php` | All pass |
...
- `tests/Feature/ItMultiGaugeLivewireTest.php` (CREATE)
...
```bash
ls resources/views/livewire/it/multi-gauge.blade.php
grep -n "multi-gauge" routes/web.php
grep -rn "Livewire::test(\"it.multi-gauge\")" tests/ | head -5
```
...
Create `tests/Feature/ItMultiGaugeLivewireTest.php` following the exemplar pattern below
...
**Verify**: `XDEBUG_MODE=off php artisan test tests/Feature/ItMultiGaugeLivewireTest.php`
...
- S1: Renders gauges with passed data props
- S2: Values update on set
- S3: Threshold color classes applied.

Use `Livewire::test('it.multi-gauge')` with `actingAs($user)`; assert state with
...
- E1: Zero/null values render safely
- E2: Values above max clamp
- E3: Missing props default.
...
New tests to write in `tests/Feature/ItMultiGaugeLivewireTest.php` (model structure on
`tests/Feature/ActivityLogPageLivewireTest.php`):

- `test_renders`
- `test_values_update`
- `test_threshold_colors`
- `test_zero_values`
...
- [ ] Focused run `XDEBUG_MODE=off php artisan test tests/Feature/ItMultiGaugeLivewireTest.php` exits 0
