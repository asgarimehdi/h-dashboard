# Plan 012: Add Livewire coverage for maps.point
...
- **Issue**: https://github.com/asgarimehdi/h-dashboard/issues/572

## Why this matters

The `maps.point` Livewire component currently has **no dedicated Livewire test
...
- **Component**: `maps.point` — single-file anonymous Livewire 4 class under
  `resources/views/livewire/` (see AGENTS.md: no `app/Livewire/*.php` files).
- **Route**: `/maps/point` — protected by `map (+auth+unit_context)` (see `routes/web.php`).
...
| Run focused | `XDEBUG_MODE=off php artisan test tests/Feature/MapsPointLivewireTest.php` | All pass |
...
- `tests/Feature/MapsPointLivewireTest.php` (CREATE)
...
```bash
ls resources/views/livewire/maps/point.blade.php
grep -n "maps/point" routes/web.php
grep -rn "Livewire::test(\"maps/point\")" tests/ | head -5
```
...
Create `tests/Feature/MapsPointLivewireTest.php` following the exemplar pattern below
...
**Verify**: `XDEBUG_MODE=off php artisan test tests/Feature/MapsPointLivewireTest.php`
...
- S1: Guest 302, no-perm 403, authed 200 with map + point form
- S2: Add point with valid coords + unit
- S3: Edit existing point
- S4: Delete point.

Use `Livewire::test('maps/point')` with `actingAs($user)`; assert state with
...
- E1: Invalid coordinates rejected
- E2: Duplicate points handled
- E3: Missing unit rejected.
...
New tests to write in `tests/Feature/MapsPointLivewireTest.php` (model structure on
...
- [ ] Focused run `XDEBUG_MODE=off php artisan test tests/Feature/MapsPointLivewireTest.php` exits 0
