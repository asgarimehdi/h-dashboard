# Plan 017: Add Livewire coverage for it.network-traffic-chart embedded component
...
- **Issue**: https://github.com/asgarimehdi/h-dashboard/issues/577

## Why this matters

The `it.network-traffic-chart (embedded)` Livewire component currently has **no dedicated Livewire test
...
- **Component**: `it.network-traffic-chart (embedded)` — single-file anonymous Livewire 4 class under
  `resources/views/livewire/` (see AGENTS.md: no `app/Livewire/*.php` files).
- **Route**: `none - embedded in IT pages` — protected by `none - standalone` (see `routes/web.php`).
...
| Run focused | `XDEBUG_MODE=off php artisan test tests/Feature/ItNetworkTrafficLivewireTest.php` | All pass |
...
- `tests/Feature/ItNetworkTrafficLivewireTest.php` (CREATE)
...
```bash
ls resources/views/livewire/it/network-traffic-chart.blade.php
grep -n "network-traffic" routes/web.php
grep -rn "Livewire::test(\"it.network-traffic-chart\")" tests/ | head -5
```
...
Create `tests/Feature/ItNetworkTrafficLivewireTest.php` following the exemplar pattern below
...
**Verify**: `XDEBUG_MODE=off php artisan test tests/Feature/ItNetworkTrafficLivewireTest.php`
...
- S1: Renders chart with data props
- S2: Time range selection updates data
- S3: Zabbix values mapped.

Use `Livewire::test('it.network-traffic-chart')` with `actingAs($user)`; assert state with
...
- E1: Empty data renders placeholder
- E2: Single data point renders
- E3: Missing series safe.
...
New tests to write in `tests/Feature/ItNetworkTrafficLivewireTest.php` (model structure on
...
- [ ] Focused run `XDEBUG_MODE=off php artisan test tests/Feature/ItNetworkTrafficLivewireTest.php` exits 0
