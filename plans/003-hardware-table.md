# Plan 003: Add Livewire coverage for hardware.table partial via hardware.index
...
- **Issue**: https://github.com/asgarimehdi/h-dashboard/issues/562

## Why this matters

The `hardware.table (partial)` Livewire component currently has **no dedicated Livewire test
...
- **Component**: `hardware.table (partial)` — single-file anonymous Livewire 4 class under
...
| Run focused | `XDEBUG_MODE=off php artisan test tests/Feature/HardwareTableLivewireTest.php` | All pass |
...
- `tests/Feature/HardwareTableLivewireTest.php` (CREATE)
...
```bash
ls resources/views/livewire/hardware/table.blade.php
grep -n "table.blade" routes/web.php
...
Create `tests/Feature/HardwareTableLivewireTest.php` following the exemplar pattern below
...
**Verify**: `XDEBUG_MODE=off php artisan test tests/Feature/HardwareTableLivewireTest.php`
...
- S1: Render mobile cards (md:hidden) + desktop x-table (hidden md:block) with seeded rows
- S2: Status badge mapping (mark->warning, off->neutral, else->success) in both layouts
- S3: Marked-row highlight
- S4: Desktop checkbox wire:model.live=selected toggles bulk selection
- S5: perPage [10,20,50,100] paginates
- S6: Sort headers reorder
- S7: editHardware(id) opens edit modal; loadHistory(id) populates; delete(id) soft-deletes.
...
- E1: Empty hardwares renders nothing, no error
- E2: mark=true + status mismatch still highlights
- E3: Unknown status falls back to active badge
- E4: Selection survives pagination/perPage change.
...
New tests to write in `tests/Feature/HardwareTableLivewireTest.php` (model structure on
`tests/Feature/ActivityLogPageLivewireTest.php`):

- `test_renders_both_layouts`
- `test_status_badges`
- `test_marked_highlight`
- `test_bulk_checkbox`
- `test_perpage_values`
- `test_sort_headers`
- `test_edit_opens_modal`
- `test_load_history`
...
- [ ] Focused run `XDEBUG_MODE=off php artisan test tests/Feature/HardwareTableLivewireTest.php` exits 0
