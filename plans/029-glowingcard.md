# Plan 029: Add Livewire coverage for glowingcard (unrouted demo component)
...
- **Issue**: https://github.com/asgarimehdi/h-dashboard/issues/590

## Why this matters

The `glowingcard (unrouted)` Livewire component currently has **no dedicated Livewire test
...
- **Component**: `glowingcard (unrouted)` — single-file anonymous Livewire 4 class under
  `resources/views/livewire/` (see AGENTS.md: no `app/Livewire/*.php` files).
- **Route**: `none - never registered, never referenced` — protected by `none` (see `routes/web.php`).
...
| Run focused | `XDEBUG_MODE=off php artisan test tests/Feature/GlowingCardLivewireTest.php` | All pass |
...
- `tests/Feature/GlowingCardLivewireTest.php` (CREATE)
...
```bash
ls resources/views/livewire/glowingcard.blade.php
grep -n "glowingcard" routes/web.php
grep -rn "Livewire::test(\"glowingcard\")" tests/ | head -5
```
...
Create `tests/Feature/GlowingCardLivewireTest.php` following the exemplar pattern below
...
**Verify**: `XDEBUG_MODE=off php artisan test tests/Feature/GlowingCardLivewireTest.php`
...
- S1: Livewire::test('glowingcard')->assertOk + quote text + author name
- S2: Static DOM: highlight div id=highlight-1, card id=card-1, Alpine x-data/mousemove/mouseleave
- S3: Zero public props/methods (no validation paths)
- S4: External logo img src preserved or flagged placeholder.

Use `Livewire::test('glowingcard')` with `actingAs($user)`; assert state with
...
- E1: If component stays unrouted, test asserts standalone render only
- E2: If deleted later, delete this test file with it (no route regression needed)
- E3: Body ASCII-only.
...
New tests to write in `tests/Feature/GlowingCardLivewireTest.php` (model structure on
...
- [ ] Focused run `XDEBUG_MODE=off php artisan test tests/Feature/GlowingCardLivewireTest.php` exits 0
