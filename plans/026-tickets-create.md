# Plan 026: Add Livewire coverage for tickets.create
...
- **Issue**: https://github.com/asgarimehdi/h-dashboard/issues/587

## Why this matters

The `tickets.create` Livewire component currently has **no dedicated Livewire test
...
- **Component**: `tickets.create` — single-file anonymous Livewire 4 class under
  `resources/views/livewire/` (see AGENTS.md: no `app/Livewire/*.php` files).
- **Route**: `/tickets/new` — protected by `create_ticket (+auth+unit_context)` (see `routes/web.php`).
...
| Run focused | `XDEBUG_MODE=off php artisan test tests/Feature/TicketsCreateLivewireTest.php` | All pass |
...
- `tests/Feature/TicketsCreateLivewireTest.php` (CREATE)
...
```bash
ls resources/views/livewire/tickets/create.blade.php
grep -n "tickets.create" routes/web.php
grep -rn "Livewire::test(\"tickets.create\")" tests/ | head -5
```
...
Create `tests/Feature/TicketsCreateLivewireTest.php` following the exemplar pattern below
...
**Verify**: `XDEBUG_MODE=off php artisan test tests/Feature/TicketsCreateLivewireTest.php`
...
- S1: Guest -> login
- S2: No-perm 403
- S3: Authed renders header + priority/subject/content/file inputs
- S4: mount loads open todos for current_unit_id
- S5: updatedSearch >=2 chars queries Unit (can_receive_tickets + is_active, excludes own, LIKE, limit 5); <2 clears + dropdown
- S6: selectUnit sets unit_id + search + hides dropdown
- S7: removeFile/resetForm clear
- S8: saveTicket success: Ticket (TK- code, created), Todo auto-created when task_id null, ActivityLog + TicketActivity, NotificationService::notifyUnit, attachments to public/attachments, success toast + reset.

Use `Livewire::test('tickets.create')` with `actingAs($user)`; assert state with
...
- E1: unit_id required/exists + own-unit custom rule (Persian message)
- E2: subject min5/max255, content min10
- E3: files: >5 rejected, mimes jpg/jpeg/png/pdf/zip/rar max5120, error bag reset
- E4: Search excludes own unit
- E5: task_id optional link vs auto-create.
...
New tests to write in `tests/Feature/TicketsCreateLivewireTest.php` (model structure on
...
- `test_search_units`
- `test_select_unit`
- `test_save_creates_ticket`
- `test_auto_creates_todo`
- `test_validation_errors`
- `test_file_validation`
- `test_own_unit_rejected`

## Done criteria

Machine-checkable. ALL must hold:

- [ ] Focused run `XDEBUG_MODE=off php artisan test tests/Feature/TicketsCreateLivewireTest.php` exits 0
