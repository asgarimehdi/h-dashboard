# Plan 008: Add Livewire coverage for kargozini.import-persons.import-persons
...
The `kargozini.import-persons.import-persons` Livewire component currently has **no dedicated Livewire test
...
- **Component**: `kargozini.import-persons.import-persons` — single-file anonymous Livewire 4 class under
  `resources/views/livewire/` (see AGENTS.md: no `app/Livewire/*.php` files).
- **Route**: `/kargozini/persons/import` — protected by `kargozini (+unit_context)` (see `routes/web.php`).
...
| Run focused | `XDEBUG_MODE=off php artisan test tests/Feature/KargoziniImportLivewireTest.php` | All pass |
...
- `tests/Feature/KargoziniImportLivewireTest.php` (CREATE)
...
```bash
ls resources/views/livewire/kargozini/import-persons/import-persons.blade.php
grep -n "kargozini.persons.import" routes/web.php
grep -rn "Livewire::test(\"kargozini.import-persons.import-persons\")" tests/ | head -5
```
...
Create `tests/Feature/KargoziniImportLivewireTest.php` following the exemplar pattern below
...
**Verify**: `XDEBUG_MODE=off php artisan test tests/Feature/KargoziniImportLivewireTest.php`
...
- S1: Mounts clean with kargozini perm
- S2: Guest 302, no-perm 403
- S3: File validation: required/mimes/max
- S4: Valid CSV -> showPreview=true, previewData + importStats populated
- S5: confirmImport without importResults -> error toast, no DB change
- S6: confirmImport after preview -> DB insert + persons-imported event + form reset
- S7: cancelImport clears all
- S8: updatedCompareKey re-preview/no-op
- S9: Per-row create->skip.

Use `Livewire::test('kargozini.import-persons.import-persons')` with `actingAs($user)`; assert state with
...
- E1: Zero rows -> total=0
- E2: Missing cols -> errors + error JSON rendered
- E3: PersonImport throws in preview -> error toast, showPreview false.
...
New tests to write in `tests/Feature/KargoziniImportLivewireTest.php` (model structure on
...
- `test_valid_preview`
- `test_confirm_without_preview`
- `test_confirm_inserts`
- `test_cancel_clears`
- `test_compare_key`
- `test_row_override`
...
- [ ] Focused run `XDEBUG_MODE=off php artisan test tests/Feature/KargoziniImportLivewireTest.php` exits 0
