# Plan 021: Add Livewire coverage for units.map (PostGIS boundary editor)
...
- **Issue**: https://github.com/asgarimehdi/h-dashboard/issues/581

## Why this matters

The `units.map` Livewire component currently has **no dedicated Livewire test
...
- **Component**: `units.map` — single-file anonymous Livewire 4 class under
  `resources/views/livewire/` (see AGENTS.md: no `app/Livewire/*.php` files).
- **Route**: `/units/{id}/map` — protected by `organization (+auth+unit_context)` (see `routes/web.php`).
...
| Run focused | `XDEBUG_MODE=off php artisan test tests/Feature/UnitsMapLivewireTest.php` | All pass |
...
- `tests/Feature/UnitsMapLivewireTest.php` (CREATE)
...
```bash
ls resources/views/livewire/units/map.blade.php
grep -n "units/{id}/map" routes/web.php
grep -rn "Livewire::test(\"units.map\")" tests/ | head -5
```
...
Create `tests/Feature/UnitsMapLivewireTest.php` following the exemplar pattern below
...
**Verify**: `XDEBUG_MODE=off php artisan test tests/Feature/UnitsMapLivewireTest.php`
...
- S1: Guest GET 302 login
- S2: No-perm 403
- S3: Authed+perm+valid id renders unit name + #unitMap container
- S4: mount unknown id -> not-found toast, hasBoundary false
- S5: saveBoundary valid Polygon GeoJSON -> boundary row created (PostGIS ST_GeomFromGeoJSON), unit.boundary_id set, boundaryUpdated dispatched
- S6: Second polygon updates same row (no duplicate)
- S7: Point/LineString/malformed JSON rejected, no DB change
- S8: deleteBoundary clears + success; no-op when none.

Use `Livewire::test('units.map')` with `actingAs($user)`; assert state with
...
- E2: Empty drawn geometry -> delete path, unit boundary-less
- E3: Existing boundary pre-renders geojson for Leaflet
- E4: Requires PostGIS test DB; rows cleaned per test (RefreshDatabase).
...
New tests to write in `tests/Feature/UnitsMapLivewireTest.php` (model structure on
...
- `test_renders`
- `test_mount_unknown`
- `test_save_polygon_creates`
- `test_save_second_updates`
...
- [ ] Focused run `XDEBUG_MODE=off php artisan test tests/Feature/UnitsMapLivewireTest.php` exits 0
