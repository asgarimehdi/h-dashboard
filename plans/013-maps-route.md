# Plan 013: Add Livewire coverage for maps.route
...
- **Issue**: https://github.com/asgarimehdi/h-dashboard/issues/573

## Why this matters

The `maps.route` Livewire component currently has **no dedicated Livewire test
...
- **Component**: `maps.route` — single-file anonymous Livewire 4 class under
  `resources/views/livewire/` (see AGENTS.md: no `app/Livewire/*.php` files).
- **Route**: `/maps/route` — protected by `map (+auth+unit_context)` (see `routes/web.php`).
...
| Run focused | `XDEBUG_MODE=off php artisan test tests/Feature/MapsRouteLivewireTest.php` | All pass |
...
- `tests/Feature/MapsRouteLivewireTest.php` (CREATE)
...
grep -n "maps/route'" routes/web.php
grep -rn "Livewire::test(\"maps/route\")" tests/ | head -5
```
...
Create `tests/Feature/MapsRouteLivewireTest.php` following the exemplar pattern below
...
**Verify**: `XDEBUG_MODE=off php artisan test tests/Feature/MapsRouteLivewireTest.php`
...
- S1: Guest 302, no-perm 403, authed 200 with map + route form
- S2: Create route with waypoints
- S3: Edit route waypoints
- S4: Delete route
- S5: Route renders polyline.

Use `Livewire::test('maps/route')` with `actingAs($user)`; assert state with
...
- E1: Single waypoint rejected
- E2: Invalid geometry rejected
- E3: Empty waypoint list.
...
New tests to write in `tests/Feature/MapsRouteLivewireTest.php` (model structure on
...
- `test_edit_route`
- `test_delete_route`
- `test_single_waypoint_rejected`

## Done criteria

Machine-checkable. ALL must hold:

- [ ] Focused run `XDEBUG_MODE=off php artisan test tests/Feature/MapsRouteLivewireTest.php` exits 0
