...
**⚠️ تأیید شد:** تقویم **overdue نشون نمیده**. Load اولیه بدون فیلتر تاریخ → همه چی load می‌شه.
تیکت‌ها بر اساس `created_at` فیلتر می‌شن نه `deadline` → تیکت قدیمی با تسک فعال ممکنه ناپدید بشه.
**راه‌حل:** اضافه کردن overdue query + فیلتر deadline + default date range برای load اولیه + styling قرمز.
