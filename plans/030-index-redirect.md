# Plan 030: Add Livewire coverage for index (role-picker redirect)
...
- **Issue**: https://github.com/asgarimehdi/h-dashboard/issues/592

## Why this matters

The `index` Livewire component currently has **no dedicated Livewire test
...
- **Component**: `index` — single-file anonymous Livewire 4 class under
  `resources/views/livewire/` (see AGENTS.md: no `app/Livewire/*.php` files).
- **Route**: `/ (name: none)` — protected by `auth+unit_context, NO permission gate` (see `routes/web.php`).
...
| Run focused | `XDEBUG_MODE=off php artisan test tests/Feature/IndexRedirectLivewireTest.php` | All pass |
...
- `tests/Feature/IndexRedirectLivewireTest.php` (CREATE)
...
```bash
ls resources/views/livewire/index.blade.php
grep -n "Route::livewire('/', 'index')" routes/web.php
grep -rn "Livewire::test(\"index\")" tests/ | head -5
```
...
Create `tests/Feature/IndexRedirectLivewireTest.php` following the exemplar pattern below
...
**Verify**: `XDEBUG_MODE=off php artisan test tests/Feature/IndexRedirectLivewireTest.php`
...
- S2: Authed + unit_context GET / 302 /dashboard
- S3: Livewire::test('index')->assertRedirect('/dashboard')
- S4: mount() short-circuits render
- S5: Default props selectedRole/roleOptions null.

Use `Livewire::test('index')` with `actingAs($user)`; assert state with
...
- E1: Authed WITHOUT unit_context -> context picker (not dashboard)
- E2: Expired session -> 302 /login, no 500
- E3: No DB writes on mount.
...
New tests to write in `tests/Feature/IndexRedirectLivewireTest.php` (model structure on
`tests/Feature/ActivityLogPageLivewireTest.php`):

- `test_guest_302`
- `test_authed_redirects_dashboard`
- `test_livewire_redirect`
...
- [ ] Focused run `XDEBUG_MODE=off php artisan test tests/Feature/IndexRedirectLivewireTest.php` exits 0
