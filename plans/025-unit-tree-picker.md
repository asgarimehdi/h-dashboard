# Plan 025: Add Livewire coverage for partials.unit-tree-picker (Alpine partial)
...
- **Issue**: https://github.com/asgarimehdi/h-dashboard/issues/586

## Why this matters

The `partials.unit-tree-picker (Alpine partial)` Livewire component currently has **no dedicated Livewire test
...
- **Component**: `partials.unit-tree-picker (Alpine partial)` — single-file anonymous Livewire 4 class under
  `resources/views/livewire/` (see AGENTS.md: no `app/Livewire/*.php` files).
- **Route**: `embedded in /kargozini/persons + /users` — protected by `kargozini or manage_users` (see `routes/web.php`).
...
| Run focused | `XDEBUG_MODE=off php artisan test tests/Feature/UnitTreePickerPartialTest.php` | All pass |
...
- `tests/Feature/UnitTreePickerPartialTest.php` (CREATE)
...
```bash
ls resources/views/livewire/partials/unit-tree-picker.blade.php
grep -n "unit-tree-picker" routes/web.php
grep -rn "Livewire::test(\"kargozini.person / users.index (hosts)\")" tests/ | head -5
```
...
Create `tests/Feature/UnitTreePickerPartialTest.php` following the exemplar pattern below
...
**Verify**: `XDEBUG_MODE=off php artisan test tests/Feature/UnitTreePickerPartialTest.php`
...
- S1: 3-level Unit tree renders all names via host component
- S2: Single select: click -> wire:model = id, dropdown closes
- S3: Multi: clicks -> array model, badge remove clears one
- S4: Search: type filters, matched highlight, ancestor auto-expand
- S5: Expand/collapse +/- toggles children.

Use `Livewire::test('kargozini.person / users.index (hosts)')` with `actingAs($user)`; assert state with
...
- E1: Empty units array -> 'not found' message
- E2: No root units
- E3: parent_id 0 vs null both handled.
...
New tests to write in `tests/Feature/UnitTreePickerPartialTest.php` (model structure on
...
- [ ] Focused run `XDEBUG_MODE=off php artisan test tests/Feature/UnitTreePickerPartialTest.php` exits 0
