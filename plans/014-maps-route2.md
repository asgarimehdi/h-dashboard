# Plan 014: Add Livewire coverage for maps.route2
...
- **Issue**: https://github.com/asgarimehdi/h-dashboard/issues/574

## Why this matters

The `maps.route2` Livewire component currently has **no dedicated Livewire test
...
- **Component**: `maps.route2` — single-file anonymous Livewire 4 class under
  `resources/views/livewire/` (see AGENTS.md: no `app/Livewire/*.php` files).
- **Route**: `/maps/route2` — protected by `map (+auth+unit_context)` (see `routes/web.php`).
...
| Run focused | `XDEBUG_MODE=off php artisan test tests/Feature/MapsRoute2LivewireTest.php` | All pass |
...
- `tests/Feature/MapsRoute2LivewireTest.php` (CREATE)
...
```bash
ls resources/views/livewire/maps/route2.blade.php
grep -n "maps/route2" routes/web.php
grep -rn "Livewire::test(\"maps/route2\")" tests/ | head -5
```
...
Create `tests/Feature/MapsRoute2LivewireTest.php` following the exemplar pattern below
...
**Verify**: `XDEBUG_MODE=off php artisan test tests/Feature/MapsRoute2LivewireTest.php`
...
- S1: Guest 302, no-perm 403, authed 200 with map + form
- S2: Create/edit/delete alternative routes
- S3: Waypoint add/remove.

Use `Livewire::test('maps/route2')` with `actingAs($user)`; assert state with
...
- E1: Empty route rejected
- E2: Boundary-condition waypoint counts
- E3: Invalid geometry.
...
New tests to write in `tests/Feature/MapsRoute2LivewireTest.php` (model structure on
...
- [ ] Focused run `XDEBUG_MODE=off php artisan test tests/Feature/MapsRoute2LivewireTest.php` exits 0
