# Plan 002: Add comprehensive Livewire coverage for units.index
...
- **Issue**: https://github.com/asgarimehdi/h-dashboard/issues/561

## Why this matters

The `units.index` Livewire component currently has **no dedicated Livewire test
...
- **Component**: `units.index` — single-file anonymous Livewire 4 class under
  `resources/views/livewire/` (see AGENTS.md: no `app/Livewire/*.php` files).
- **Route**: `/units` — protected by `organization` (see `routes/web.php`).
...
| Run focused | `XDEBUG_MODE=off php artisan test tests/Feature/UnitsIndexLivewireTest.php` | All pass |
...
- `tests/Feature/UnitsIndexLivewireTest.php` (CREATE)
...
```bash
ls resources/views/livewire/units/index.blade.php
grep -n "units.index" routes/web.php
grep -rn "Livewire::test(\"units.index\")" tests/ | head -5
```
...
Create `tests/Feature/UnitsIndexLivewireTest.php` following the exemplar pattern below
...
**Verify**: `XDEBUG_MODE=off php artisan test tests/Feature/UnitsIndexLivewireTest.php`
...
- S1: Smoke: auth+perm 200 with headers, no-perm 403
- S2: Search by name LIKE; perPage toggle; column sorting
- S3: Dropdown cascading: updatedUnitTypeId resets province/region/parent; updatedProvinceId resets region/parent + reloads counties; updatedRegionId resets parent
- S4: Create (valid persist + toast), edit (populate + save), validation errors, invalid parent type toast, county-level fixed region
- S5: Delete success + FK-referenced error toast
- S6: Level logic: ministry sees provinces, province sees own counties, county auto-set
- S7: Toggle ticket capability.

Use `Livewire::test('units.index')` with `actingAs($user)`; assert state with
...
- E1: Delete unit with FK references
- E2: Invalid parent type not in UnitTypeRelationship
- E3: County user sees no dropdowns
- E4: Empty unit list.
...
New tests to write in `tests/Feature/UnitsIndexLivewireTest.php` (model structure on
`tests/Feature/ActivityLogPageLivewireTest.php`):

- `test_authorized_renders`
- `test_unauthorized_403`
- `test_search_by_name`
- `test_perpage_toggle`
- `test_sorting`
- `test_dropdown_cascading`
- `test_create_unit`
- `test_edit_unit`
- `test_validation_errors`
- `test_delete_unit`
- `test_delete_fk_blocked`
- `test_level_logic`
- `test_toggle_ticket_capability`

## Done criteria

Machine-checkable. ALL must hold:

- [ ] Focused run `XDEBUG_MODE=off php artisan test tests/Feature/UnitsIndexLivewireTest.php` exits 0
