# Plan 005: Add Livewire coverage for hardware.audit-modal partial via hardware.index
...
- **Issue**: https://github.com/asgarimehdi/h-dashboard/issues/565

## Why this matters

The `hardware.audit-modal (partial)` Livewire component currently has **no dedicated Livewire test
...
- **Component**: `hardware.audit-modal (partial)` — single-file anonymous Livewire 4 class under
...
| Run focused | `XDEBUG_MODE=off php artisan test tests/Feature/HardwareAuditModalLivewireTest.php` | All pass |
...
- `tests/Feature/HardwareAuditModalLivewireTest.php` (CREATE)
...
```bash
ls resources/views/livewire/hardware/audit-modal.blade.php
grep -n "audit-modal" routes/web.php
...
Create `tests/Feature/HardwareAuditModalLivewireTest.php` following the exemplar pattern below
...
**Verify**: `XDEBUG_MODE=off php artisan test tests/Feature/HardwareAuditModalLivewireTest.php`
...
- S1: loadHistory sets showHistoryModal + hardwareId + history
- S2: Empty history shows message
- S3: filterHistory (null/created/updated/deleted/bulk_mark/bulk_delete/rollback) filters; active button btn-primary
- S4: Pagination: historyPage(n), prev disabled page 1, next disabled last, indicator correct
- S5: Per-field rollback button only for updated/rollback with old != em-dash; rollbackHistoryField restores + creates audit + notifies
- S6: New loadHistory/filter resets page 1.
...
- E1: No matching action -> empty state
- E2: historyTotal<=perPage hides pagination
- E3: Invalid audit id / mismatched hardware_id: no crash, error, modal stays open
- E4: Wrong unit -> empty/403
- E5: Jalali date renders valid Y/m/d H:i per entry.
...
New tests to write in `tests/Feature/HardwareAuditModalLivewireTest.php` (model structure on
`tests/Feature/ActivityLogPageLivewireTest.php`):

- `test_load_history_opens_modal`
- `test_empty_history_message`
- `test_filter_history_actions`
- `test_pagination_controls`
- `test_rollback_button_visibility`
- `test_rollback_restores_field`
- `test_page_resets`
...
- [ ] Focused run `XDEBUG_MODE=off php artisan test tests/Feature/HardwareAuditModalLivewireTest.php` exits 0
