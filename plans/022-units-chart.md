# Plan 022: Add Livewire coverage for units.chart org tree
...
- **Issue**: https://github.com/asgarimehdi/h-dashboard/issues/582

## Why this matters

The `units.chart` Livewire component currently has **no dedicated Livewire test
...
- **Component**: `units.chart` — single-file anonymous Livewire 4 class under
  `resources/views/livewire/` (see AGENTS.md: no `app/Livewire/*.php` files).
- **Route**: `/units/chart` — protected by `organization (+auth+unit_context)` (see `routes/web.php`).
...
| Run focused | `XDEBUG_MODE=off php artisan test tests/Feature/UnitsChartLivewireTest.php` | All pass |
...
- `tests/Feature/UnitsChartLivewireTest.php` (CREATE)
...
```bash
ls resources/views/livewire/units/chart.blade.php
grep -n "units/chart" routes/web.php
grep -rn "Livewire::test(\"units.chart\")" tests/ | head -5
```
...
Create `tests/Feature/UnitsChartLivewireTest.php` following the exemplar pattern below
...
**Verify**: `XDEBUG_MODE=off php artisan test tests/Feature/UnitsChartLivewireTest.php`
...
- S1: Auth+perm sees header + root unit name; guest 302
- S2: rootUnits only accessible units
- S3: toggle(id) expands then collapses
- S4: search >2 chars auto-expands parent chain; <=2 leaves expanded empty
- S5: selectUnit(accessible) sets selectedUnit + directUserCount/descendantUserCount + detail card
- S6: selectUnit(out-of-scope) keeps null + error toast (no leak).

Use `Livewire::test('units.chart')` with `actingAs($user)`; assert state with
...
- E1: User with no accessible roots sees empty-state text
- E2: Passes serial and parallel.
...
New tests to write in `tests/Feature/UnitsChartLivewireTest.php` (model structure on
...
- [ ] Focused run `XDEBUG_MODE=off php artisan test tests/Feature/UnitsChartLivewireTest.php` exits 0
