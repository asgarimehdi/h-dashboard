# Plan 004: Add Livewire coverage for hardware.filters partial via hardware.index
...
- **Issue**: https://github.com/asgarimehdi/h-dashboard/issues/563

## Why this matters

The `hardware.filters (partial)` Livewire component currently has **no dedicated Livewire test
...
- **Component**: `hardware.filters (partial)` — single-file anonymous Livewire 4 class under
...
| Run focused | `XDEBUG_MODE=off php artisan test tests/Feature/HardwareFiltersLivewireTest.php` | All pass |
...
- `tests/Feature/HardwareFiltersLivewireTest.php` (CREATE)
...
```bash
ls resources/views/livewire/hardware/filters.blade.php
grep -n "filters.blade" routes/web.php
...
Create `tests/Feature/HardwareFiltersLivewireTest.php` following the exemplar pattern below
...
**Verify**: `XDEBUG_MODE=off php artisan test tests/Feature/HardwareFiltersLivewireTest.php`
...
- S1: Filters hidden by default (showFilters=false); toggling reveals panel
- S2: toggleFilter(filterType/laptop) sets, repeat clears
- S3: toggleFilter(filterRam/16384) affects query
- S4: clearFilters resets ALL 11 properties
- S5: hasActiveFilters true/false
- S6: loadDeletedHardware toggles deleted view
- S7: Combining type+ram+shutdown narrows results
- S8: filterPerson/Unit/Semat use normalizeForSearch.
...
- E1: Toggle same value twice = OFF (idempotent)
- E2: clearFilters with none set = no-op
- E3: filterShutdown '1' maps boolean true
...
New tests to write in `tests/Feature/HardwareFiltersLivewireTest.php` (model structure on
`tests/Feature/ActivityLogPageLivewireTest.php`):

- `test_filters_hidden_by_default`
- `test_toggle_panel`
- `test_toggle_filter_set`
- `test_toggle_filter_off`
- `test_clear_filters`
- `test_has_active_filters`
- `test_deleted_view`
...
- [ ] Focused run `XDEBUG_MODE=off php artisan test tests/Feature/HardwareFiltersLivewireTest.php` exits 0
