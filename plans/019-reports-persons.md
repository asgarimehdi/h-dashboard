# Plan 019: Add Livewire coverage for reports.persons
...
- **Issue**: https://github.com/asgarimehdi/h-dashboard/issues/579

## Why this matters

The `reports.persons` Livewire component currently has **no dedicated Livewire test
...
- **Component**: `reports.persons` — single-file anonymous Livewire 4 class under
  `resources/views/livewire/` (see AGENTS.md: no `app/Livewire/*.php` files).
- **Route**: `/reports/persons` — protected by `auth+unit_context` (see `routes/web.php`).
...
| Run focused | `XDEBUG_MODE=off php artisan test tests/Feature/ReportsPersonsLivewireTest.php` | All pass |
...
- `tests/Feature/ReportsPersonsLivewireTest.php` (CREATE)
...
```bash
ls resources/views/livewire/reports/persons.blade.php
grep -n "reports/persons" routes/web.php
grep -rn "Livewire::test(\"reports.persons\")" tests/ | head -5
```
...
Create `tests/Feature/ReportsPersonsLivewireTest.php` following the exemplar pattern below
...
**Verify**: `XDEBUG_MODE=off php artisan test tests/Feature/ReportsPersonsLivewireTest.php`
...
- S1: Guest 302, no unit_context -> select-context, authed 200 with stats
- S2: Person counts match seeded data
- S3: Charts render
- S4: Scope via AccessService filters to subtree.

Use `Livewire::test('reports.persons')` with `actingAs($user)`; assert state with
...
- E1: Empty accessibleUnitIds -> zeros, no error
- E2: Zero persons
- E3: Units outside scope excluded.
...
New tests to write in `tests/Feature/ReportsPersonsLivewireTest.php` (model structure on
...
- [ ] Focused run `XDEBUG_MODE=off php artisan test tests/Feature/ReportsPersonsLivewireTest.php` exits 0
